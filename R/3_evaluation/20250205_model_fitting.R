...
# Simulations
simulations <- rast(paste0(wd, '/data/20250507_sim_ae_raster001.tif'))
simulations <- st_as_sf(as.data.frame(simulations, xy=T), crs = 32650, coords = c('x', 'y'))
lc_values <- terra::extract(landcover, simulations, xy = T)
simulations$cat_500m <- round(lc_values$Landcover_AllClass)
simulations$cat_500m <- plyr::revalue(as.character(simulations$cat_500m), c("0"="C_Oil", "1"="B_Secondary", "2"="A_Primary", "3"="D_Plantation", "4"="E_Built"))
...
# Samples
samples <- st_read(paste0(wd, '/data/20250507_points_sampling_scenarios_ae_alldata.geojson'))
samples$cat_500m <- plyr::revalue(as.character(samples$land_cover), c("0"="C_Oil", "1"="B_Secondary", "2"="A_Primary", "3"="D_Plantation", "4"="E_Built"))
...
s <- as.numeric(commandArgs(trailingOnly = TRUE)[2])
